import React from "react";
import ServicesCommonComponent from "./ServicesCommonComponent";
import itSupportAndManageService from "../../Components/images/services/itSupportAndManageService.avif";
const ITSupportAndManagedServices = () => {
  return (
    <section className="bg-gray-100 py-12 px-6 md:px-12">
      <div className="max-w-6xl mx-auto text-center">
        <ServicesCommonComponent />
        <h3 className="text-2xl font-semibold text-gray-700 mb-6 text-start">
          IT Support & Managed Services
        </h3>
        <p className="text-gray-600 text-base mb-5 text-start">
          Our <strong>24/7 IT support</strong> ensures business continuity with
          proactive monitoring, troubleshooting, and system optimization.
        </p>
        <div className="grid md:grid-cols-2 gap-8 text-left">
          <div className="p-6 bg-white shadow-md rounded-lg max-w-sm">
            <img
              src={itSupportAndManageService}
              alt="big-data"
              className="w-full h-auto rounded-md bg-no-repeat object-cover"
            />
            <ul className="list-disc pl-5 text-gray-600 text-base mt-4">
              <li>Network management & security</li>
              <li>Server administration & cloud hosting</li>
              <li>Helpdesk & user support</li>
            </ul>
          </div>
        </div>
      </div>
    </section>
  );
};

export default ITSupportAndManagedServices;